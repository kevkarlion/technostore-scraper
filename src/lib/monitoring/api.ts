...
export function createMonitoringRouter(
   ...
   */
  function argentinaHourOf(d: Date): number {
  ...
  router.get('/status', async (req: any, res: any) => {
    try {
      const lastExecution = await execCollection.findOne(
        {},
        { sort: { startedAt: -1 } }
      ) as ExecutionLog | null;
      ...
      let scraperStatus: DashboardStatus['scraperStatus'] = 'idle';
      if (lastExecution) {
        const now = new Date();
      ...
      const last7Days = await execCollection.find(
      ...
      let lastExecutionTimeAgo: string | null = null;
      if (lastExecution?.startedAt) {
        const diffMs = Date.now() - lastExecution.startedAt.getTime();
        const diffMin = Math.floor(diffMs / 60000);
        if (diffMin < 1) lastExecutionTimeAgo = 'Just now';
        else if (diffMin < 60) lastExecutionTimeAgo = `${diffMin}m ago`;
        else lastExecutionTimeAgo = `${Math.floor(diffMin / 60)}h ${diffMin % 60}m ago`;
      }
   ...
   */
  router.get('/executions/:id', async (req: any, res: any) => {
    try {
      let objectId;
      try {
  ...
  router.get('/metrics', async (req: any, res: any) => {
    try {
      ...
      if (metricsAggregator && typeof metricsAggregator.getMetrics === 'function') {
        ...
        snapshots = await metricsCollection.find(
          { date: { $gte: sinceStr } },
          { sort: { date: -1 } }
        ).toArray();
      }
      ...
      const avgDurationMs = daysWithRuns > 0
        ? Math.round(
            ascending.reduce((s, m) => s + (m.avgDurationMs || 0), 0) / ascending.length
          )
        : 0;
      const avgProductsFound = daysWithRuns > 0
        ? Math.round(
            ascending.reduce((s, m) => s + (m.avgProductsFound || 0), 0) / ascending.length
          )
        : 0;
      const avgSuccessRate = ascending.length > 0
        ? Math.round(
            ascending.reduce((s, m) => {
  ...
  router.get('/health', async (_req: any, res: any) => {
    try {
      ...
      if (healthChecker && typeof healthChecker.getActiveAlerts === 'function') {
        ...
        alerts = await healthCollection.find(
   ...
   * one is detected.
   */
  router.post('/health/check', async (_req: any, res: any) => {
    try {
      const detected: any[] = [];
