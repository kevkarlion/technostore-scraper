 ...
 */
function createMetricsAggregator(config) {
    const db = config.db;
    ...
    async function getMetrics(days = 7) {
        try {
            ...
            return await metricsCollection.find({ date: { $gte: sinceStr } }, { sort: { date: -1 } }).toArray();
        }
        catch {
            return [];
        }
     ...
     */
    function startPeriodicAggregation(intervalMs = 60 * 60 * 1000) {
