    ...
    async bulkUpsert(products) {
        ...
        for (const product of products) {
            ...
            if (!existing) {
                ...
                // Check cambios
                const changes = { lastSyncedAt: now };
...
const scraperRunRepository = {
    async create(run) {
        ...
        const runId = run.runId || crypto_1.default.randomUUID();
        const now = new Date();
    ...
    async cleanupStaleRuns(hoursOld) {
        const db = await getDb();
        const cutoff = new Date(Date.now() - hoursOld * 60 * 60 * 1000);
        const result = await db.collection('scraper_runs').updateMany({ status: 'in_progress', updatedAt: { $lt: cutoff } }, { $set: { status: 'stale' } });
        return result.modifiedCount;
    },
    ...
    async incrementResumeCount(runId) {
        const db = await getDb();
        await db.collection('scraper_runs').updateOne({ runId }, { $inc: { resumeCount: 1 }, $set: { updatedAt: new Date() } });
    },
    async markCompleted(runId, stats) {
        const db = await getDb();
        await db.collection('scraper_runs').updateOne({ runId }, {
            $set: {
                status: 'completed',
    ...
    async markFailed(runId, error) {
        const db = await getDb();
        await db.collection('scraper_runs').updateOne({ runId }, { $set: { status: 'failed', errorMessage: error, updatedAt: new Date() } });
    },
    async updateCheckpoint(runId, checkpoint) {
        const db = await getDb();
        await db.collection('scraper_runs').updateOne({ runId }, { $set: { ...checkpoint, updatedAt: new Date() } });
    }
};
...
class ScraperService {
     ...
     */
    async safeGoto(page, url, retries = MAX_RETRIES) {
     ...
     */
    async safeContent(page, retries = MAX_RETRIES) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                if (!page || page.isClosed()) {
                    return null;
    ...
    async login(page) {
        try {
            ...
            throw new types_1.ScraperError(`Login failed: ${message}`, "AUTH_FAILED", error);
        }
    }
    /**
     ...
     */
    async scrapeProductDetail(page, productUrl) {
        try {
     ...
     */
    async scrapeProducts(page, categoriesToProcess) {
        const allProducts = [];
        const seenExternalIds = new Set(); // Track IDs to avoid duplicates
        ...
        for (let catIndex = 0; catIndex < categoriesToProcess.length; catIndex++) {
            ...
            try {
                ...
                await page.goto(`${this.config.baseUrl}/buscar.aspx?idsubrubro1=${category.idsubrubro1}&pag=${pageNum}`, { waitUntil: "networkidle" });
                ...
                while (hasNextPage) {
                    ...
                    try {
                        ...
                        const navSuccess = await this.safeGoto(paginationPage, `${this.config.baseUrl}/buscar.aspx?idsubrubro1=${category.idsubrubro1}&pag=${nextPageNum}`);
                        if (!navSuccess) {
                            hasNextPage = false;
     ...
     */
    async scrapeProductsInParallel(pageProducts) {
        const results = [];
    ...
    async run() {
        ...
        try {
            ...
            const { jotakpCategories } = await Promise.resolve().then(() => __importStar(require("./config")));
            ...
            const { products, errors } = (0, data_transformer_1.transformProducts)(rawProducts, this.config.supplier);
            ...
            for (let i = 0; i < products.length; i++) {
                ...
                if (product.imageUrls && product.imageUrls.length > 0) {
                    ...
                    try {
                        ...
                        const cloudUrls = await (0, image_downloader_1.uploadProductImages)(product.imageUrls, product.supplier, product.externalId);
            ...
            if (this.request.idsubrubro1 !== undefined) {
                ...
                discontinuedCount = await productRepository.markDiscontinued(this.config.supplier, seenExternalIds);
            }
            else {
     ...
     */
    async saveCheckpoint(category, pageNum) {
        if (!this.currentRun)
            return;
        await scraperRunRepository.updateCheckpoint(this.currentRun.runId, {
